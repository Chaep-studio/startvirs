...
import { getToolServerUrl, getFileMode } from '../agent/tools';
...
export default function FileExplorer({ serverUrl = getToolServerUrl() }: Props) {
  ...
  /** 根据当前 fileMode 选择 API 路径：test 模式走 /api/test/，其余走 /api/tools/ */
  const apiBase = () => getFileMode() === 'test' ? `${serverUrl}/api/test` : `${serverUrl}/api/tools`;

  const loadDir = useCallback(async (dirPath: string) => {
    setLoading(true);
    setError('');
    try {
      const res = await fetch(`${apiBase()}/list_files`, {
        method: 'POST',
  ...
  const loadFile = useCallback(async (filePath: string) => {
    ...
    setPreviewFile(null);
    try {
      const res = await fetch(`${apiBase()}/read_file`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
